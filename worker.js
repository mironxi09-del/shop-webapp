...
export default {
  async fetch(request,env) {
    ...
    }    if (request.method==='GET' && path==='/clear-telegram-queue') {
      try {
        await telegram(env,'setWebhook',{url:new URL('/telegram',request.url).toString(),secret_token:env.WEBHOOK_SECRET,allowed_updates:['message','callback_query'],drop_pending_updates:true});
        return new Response('Telegram queue cleared and webhook restored.');
      } catch { return new Response('Queue clearing failed.',{status:503}); }
